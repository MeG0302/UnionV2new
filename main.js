...
async function bridgeUSDC(wallet, maxTransaction = 1) {
  try {
    ...
    const amountToBridge = balance.gt(ethers.utils.parseUnits(maxTransaction.toString(), decimals))
      ? ethers.utils.parseUnits(maxTransaction.toString(), decimals)
      : balance;

    const allowance = await usdc.allowance(address, SEPOLIA_BRIDGE_ADDRESS);
    if (allowance.lt(amountToBridge)) {
      const approveTx = await usdc.approve(SEPOLIA_BRIDGE_ADDRESS, amountToBridge);
      const approveReceipt = await approveTx.wait();
      console.log(`Approved ${ethers.utils.formatUnits(amountToBridge, decimals)} USDC for bridging from ${address}`);
      console.log(`Gas used for approval: ${approveReceipt.gasUsed.toString()}`);
    }

    const transferTx = await bridge.transferToBabylon(SEPOLIA_USDC_ADDRESS, amountToBridge, babyAddress);
    const transferReceipt = await transferTx.wait();
    console.log(`Bridged ${ethers.utils.formatUnits(amountToBridge, decimals)} USDC from ${address} to ${babyAddress} on Babylon.`);
    console.log(`Gas used for transfer: ${transferReceipt.gasUsed.toString()}`);
  } catch (err) {
